"""
FastAPI Intent Classifier with LangChain + Gemini
Replaces the hardcoded Code1 node logic in n8n with AI-powered intent detection
"""
...
app = FastAPI(
    title="WhatsApp Intent Classifier",
    description="AI-powered intent detection for WhatsApp customer service using LangChain + Gemini",
    version="1.0.0"
)
...
def classify_intent_with_ai(
    ...
    prompt = PromptTemplate(
        template="""You are an expert intent classifier for a WhatsApp-based e-commerce customer service system for "Star Trade" company.
...
- Common Hindi terms: "mil jayega", "kuch aur", "stock khatam", "order kardo", etc.

{format_instructions}

Classify the intent based on the context and message above.""",
        input_variables=["user_message", "message_type", "active_flow", "last_message"],
...
async def classify_intent(request: IntentRequest):
    ...
    try:
        ...
        check_if_quick_message = bool(msg.message_context or (msg.button_text and msg.button_payload))
...
if __name__ == "__main__":
    import uvicorn
    # Use import string so uvicorn can enable the --reload feature when
    # the app is started via `python main.py`.
